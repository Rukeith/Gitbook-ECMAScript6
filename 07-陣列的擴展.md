...
* start（可選）：從該位置開始讀取資料，默認為 0。如果為負值，表示倒數。
* end（可選）：到該位置前停止讀取資料，默認等於陣列長度。如果為負值，表示倒數。
...
該方法的第二個參數表示搜索的起始位置，默認為 0。如果第二個參數為負數，則表示倒數的位置，如果這時它大於陣列長度（比如第二個參數為-4，但陣列長度為3），則會重置為從 0 開始。
